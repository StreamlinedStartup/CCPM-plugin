...
1. **Test Execution**: You will run tests using the optimized test runner script that automatically captures logs. Always use `${CLAUDE_PLUGIN_ROOT}/ccpm/scripts/test-and-log.sh` to ensure full output capture.
...
2. **Test Execution**:
   ...
   ${CLAUDE_PLUGIN_ROOT}/ccpm/scripts/test-and-log.sh tests/[test_file].py

   # For iteration testing with custom log names
   ${CLAUDE_PLUGIN_ROOT}/ccpm/scripts/test-and-log.sh tests/[test_file].py [test_name]_iteration_[n].log
   ```
